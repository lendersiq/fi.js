...
function loadLibraryScripts(filePaths, callback) {
  ...
  function accessIndexedDB(storeName, data, mode = 'readwrite', callback) {
  ...
  function handleAPICache(filePath, callback) {
    const storeName = getFilenameWithoutExtension(filePath);
    const now = new Date().getTime();

    accessIndexedDB(storeName, null, 'readonly', (store) => {
      const getRequest = store.get(storeName);
  ...
  filePaths.forEach(filePath => {
    if (typeof filePath === 'string' && (filePath.startsWith('http://') || filePath.startsWith('https://'))) {
      handleAPICache(filePath, (data) => {
        const libName = getFilenameWithoutExtension(filePath);
        window.libraries.api = window.libraries.api || {};
        window.libraries.api[libName] = data;
        console.log(`API library '${libName}' loaded:`, data);
        scriptLoaded();
      });
    } else if (!document.querySelector(`script[src="../../libraries/${filePath}.js"]`)) {
      const script = document.createElement('script');
      ...
      console.warn(`Script ${filePath} is already loaded.`);
      scriptLoaded();
    }
  });
}
